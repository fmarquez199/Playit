...
- [**Comments and White spaces**](#comments-and-white-spaces)

---
---
# **Block and Scope**

A block within the program is delimited by the name that represents the instruction 
and at the end **`. ~`**.

All Flow control structures, subroutines, records and the main program represents a block and a scope.

The scope is static (or lexical), this means that within each scope if a reference 
is made to a variable that is not within that scope, the scope where the current 
block was defined is searched and so on, recursively, until reaching global scope.
...
[**Next page ->**](05-Instructions.md/#index)
